...
part2 :: [String] -> IO ()
part2 inputLines = print(foldr (curry changeStateOfConnectionsAfterGrouping) (0, communications) [0..1999])
        where communications = getInitialAdjacencyLists inputLines
...
changeStateOfConnectionsAfterGrouping (currentNumber, (totalGroups, connections)) = (newAmountOfGroups, remainingConnections)
        where stringOfParent = show currentNumber
              ...
              remainingConnections = filter (\(key, communicators) -> key `notElem` foundGroupOfCurrentNumber) connections
...
getChildren (searchParent, communications, foundAlready) = nub allChildren
        where (p, directChildren) = head (filter (\connection -> isKeyEqualToDesiredKey(searchParent,connection)) communications)
